...
| 고현아 | 전처리 단계에서 데이터 누수를 완벽히 차단하기 위해, 꼼꼼하게 코드를 검증하고 논리적 구조를 다듬는 과정에서 큰 기술적 챌린지를 겪었습니다. 비록 과정은 까다로웠지만 이를 극복하며 데이터 정제의 실무적 중요성을 배웠고, 분석 결과가 실제 웹 화면까지 연결되어 한 프로젝트를 완성한다는 것에 대한 전체도를 알 수 있었던것 같아요.|
